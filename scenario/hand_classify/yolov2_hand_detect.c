...
#define MODEL_FILE_HAND    "/userdata/models/hand_classify/yolov5_320.wk" // darknet framework wk model
...
static HI_S32 HandDetect(uintptr_t model, IVE_IMAGE_S *srcYuv, DetectObjInfo boxs[])
{
    ...
    int objNum;
    int ret = Yolo5CalImg(self, srcYuv, boxs, DETECT_OBJ_MAX, &objNum);
    if (ret < 0) {
